  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setStats((prev) => ({
        ...prev,
        activeIncidents:
          prev.activeIncidents + Math.floor(Math.random() * 3) - 1,
        sosAlerts: Math.max(
          0,
          prev.sosAlerts + Math.floor(Math.random() * 2) - 1
        ),
      }));
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <Card className="shadow-card">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">
              Digital IDs Verified
            </CardTitle>
            <Users className="h-4 w-4 text-success" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">
              {stats.verifiedIds.toLocaleString()}
            </div>
            <p className="text-xs text-muted-foreground">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card className="shadow-card">
          <CardHeader>
            ...
            <CardDescription>Current risk assessment by zone</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            {riskZones.map((zone) => (
              <div
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      {zone.incidentCount} incidents • Updated{" "}
                      {formatTimeAgo(zone.lastUpdate)}
                    </p>
                  </div>
                </div>
                <Badge
